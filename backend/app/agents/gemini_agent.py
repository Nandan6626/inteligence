...
def run_gemini_agent(company_data: dict, missing_fields: list = None) -> str:
    # Actually use Google Gemini here
    ...
    try:
        ...
        search_context = search.invoke(f"{company_name} latest developments, technology, and market position")
    ...
    try:
        ...
        print(f"Gemini Agent failed with error: {e}. Attempting fallback to Cerebras...")
        try:
            from langchain_cerebras import ChatCerebras
            fallback_llm = ChatCerebras(model="llama3.1-8b", temperature=0.2, api_key=settings.CEREBRAS_API_KEY)
            response = fallback_llm.invoke(formatted_prompt)
            print("Fallback to Cerebras successful!")
            return response.content.strip()
        except Exception as fallback_err:
            print(f"Cerebras fallback also failed: {fallback_err}")
